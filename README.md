...
| [`conformance/`](conformance) | The **cross-implementation conformance suite**: a shared conformance guest run by per-target adapters (wasmtime, jco under Node and headless Chrome, the composed wasip3 stack), a non-wasm reference peer (plain Node over @roamhq/wrtc) every target is paired against, an netns lab, a Shadow lab, and a runner that renders the results matrix. `just conformance`; see [`conformance/README.md`](conformance/README.md). |
